    ...
    <div class="wrapper md:p-4 space-y-6">
        ...
        <section>
            ...
            <div class="w-full mx-auto grid md:grid-cols-2 lg:grid-cols-3 gap-2 md:gap-4 pt-6 pb-2 dark:bg-slate-900/50 bg-white/30">
             <CardClassRoom v-for="i in 5" :key="i" :id="i"/>
             <router-link to="#">
                    <div class="w-full h-full flex flex-col group items-center justify-center p-4 space-y-4 hover:border-sky-500 dark:hover:border-sky-500 transition bg-white dark:bg-dark-blue border-2 border-dashed border-slate-700/10 card-shadow-sm min-h-[150px] rounded dark:border-slate-700/50">
            ...
            <div v-if="$route.query.sajda != 'true' && !state.isLoading && state.isLogin && state.ayah" class="hidden items-center my-4 justify-center">
