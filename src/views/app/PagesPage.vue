    ...
    <div class="wrapper md:p-4 space-y-6">
        ...
        <section class="grid lg:grid-cols-2">
            <div class="bg-white card-shadow-md dark:bg-dark-blue ring-1 dark:ring-slate-700/50 ring-slate-700/10 rounded p-4 flex flex-col">
                <div class="w-full flex items-center justify-between border-b dark:border-slate-700/75 pb-2 px-1">
                    ...
                    <p class="text-sm hidden md:block text-slate-500 dark:text-slate-50">Terdapat total <span class="text-sky-500 font-semibold">604</span> Page</p>
                </div>
                <div class="pt-4">
                    <p class="text-white rounded bg-sky-500 w-max py-1 px-2 mb-2 text-sm">Info</p>
                    <p class="text-slate-600 dark:text-slate-100">Halaman ini berisi metadata semua page, dengan informasi nomor halaman, letak surah, dan nomor ayah.</p>
        ...
        <section>
            <div class="w-full flex items-center justify-between border-b border-slate-700/20 dark:border-slate-700/75 pb-2 px-1">
                ...
                <p class="text-sm hidden md:block text-slate-500 dark:text-slate-50">Kamu dapat mencari semua metadata page disini</p>
            </div>
            ...
            <div class="w-full grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 pt-6 pb-2">
                <CardPageMetadata 
                    v-for="page in state.pageMetadata" 
                    :key="page.page"
                    :page="page"
                />
            </div>
...
const state = reactive({
    pageMetadata: computed(() => pageService.pages),
    lastVisible: computed(() => pageService.lastPageVisible),
    isLast: computed(()=> pageService.isLast),
    isPush: computed(()=> pageService.isPush),
    isLoading: computed(()=> pageService.isLoading)
});

onMounted(()=> {
    if(!state.pageMetadata.length)
...
const nextPage = ()=>{
    pageService.nextPage({lastVisible: state.lastVisible as any});
}
