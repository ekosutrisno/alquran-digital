        ...
        <div class="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
          ...
          <Menu as="div" class="ml-3 relative z-50">
            ...
            <transition enter-active-class="transition ease-out duration-100" enter-from-class="opacity-0 scale-95" enter-to-class="opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="opacity-100 scale-100" leave-to-class="opacity-0 scale-95">
              <MenuItems class="origin-top-right absolute right-0 mt-2 w-64 rounded-md shadow-lg overflow-hidden bg-white dark:bg-slate-900 ring-1 ring-slate-700/5 dark:ring-slate-700/50 focus:outline-none">
                <MenuItem v-for="notif in state.haveNotification" :key="notif.timestamp" v-slot="{ active }">
                  <router-link to="/" :class="[active ? 'bg-slate-100/50 dark:bg-slate-900' : '', 'inline-flex items-start space-x-2 w-full px-4 py-3 text-slate-900 dark:text-white']">
                    <BellIcon class="h-6 w-6 text-sky-300" aria-hidden="true" />
                    <div class="flex flex-col">
                      <span class="text-xs font-semibold">{{ notif.text }}</span>
                      <span class="text-xs text-slate-100">{{ notif.data }}</span>
                    </div>
                  </router-link>
                </MenuItem>
          ...
          <Menu as="div" class="ml-3 relative z-50">
            ...
            <transition enter-active-class="transition ease-out duration-100" enter-from-class="opacity-0 scale-95" enter-to-class="opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="opacity-100 scale-100" leave-to-class="opacity-0 scale-95">
              <MenuItems class="origin-top-right absolute right-0 mt-2 w-64 rounded-md shadow-lg overflow-hidden bg-white dark:bg-slate-900 ring-1 ring-slate-700/10 dark:ring-slate-700/50 focus:outline-none">
                <MenuItem>
                  <div :class="['p-4 relative h-full flex flex-col space-y-2 items-center justify-center bg-white border-b border-slate-700/10 dark:border-slate-700/50 dark:bg-slate-900 text-slate-800 dark:text-white']">
                     ...
                     <Svg2 aria-hidden="true" class="absolute -top-2"/>
                      <button type="button" class="absolute top-2 right-5">
                          <svg v-if="isDark" @click="togleDarkLightMode()" xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-[#9a6fc3]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z" />
                          </svg>
                          <svg v-else @click="togleDarkLightMode()" xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-[#9a6fc3]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <MenuItem v-slot="{ active }">
                  <router-link to="/u/0/settings" :class="[active ? 'bg-slate-100 dark:bg-slate-800' : '', 'inline-flex items-center space-x-2 w-full px-4 py-3 text-slate-600 font-medium dark:text-white']">
                ...
                <MenuItem v-if="true" v-slot="{ active }">
                  <router-link to="/a/0/dashboard" :class="[active ? 'bg-slate-100 dark:bg-slate-800' : '', 'inline-flex items-center space-x-2 w-full px-4 py-3 text-slate-600 font-medium dark:text-white']">
                ...
                <MenuItem v-slot="{ active }">
                  <router-link to="/user/help" :class="[active ? 'bg-slate-100 dark:bg-slate-800' : '', 'inline-flex items-center space-x-2 w-full px-4 py-3 text-slate-600 font-medium dark:text-white']">
                    <svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" class="text-gray-400 h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <MenuItem v-slot="{ active }">
                  <button @click="onLogoutAction" :class="[active ? 'bg-slate-100 dark:bg-slate-800' : '', 'inline-flex items-center space-x-2 w-full px-4 py-3 text-slate-600 font-medium dark:text-white']">
                    <svg xmlns="http://www.w3.org/2000/svg" aria-hidden="true" class="text-gray-400 h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
