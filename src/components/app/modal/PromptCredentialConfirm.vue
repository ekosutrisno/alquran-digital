    ...
    <Dialog as="div" class="relative z-10 font-quicksand" @close="closeModal">
      ...
      <div class="fixed z-10 inset-0 overflow-y-auto">
        <div class="flex w-full items-start justify-center min-h-full p-4 sm:p-0">
            <DialogPanel class="relative w-full rounded-2xl overflow-hidden transform transition-all sm:my-8 sm:max-w-(--breakpoint-md) sm:w-full">
              <div class="bg-white flex flex-col mx-auto max-w-xl rounded-2xl justify-between dark:bg-slate-800 border-2 border-transparent dark:border-slate-700/75">
                  <div class="ss px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                      <div class="mx-auto shrink-0 flex items-center justify-center h-12 w-12 rounded-full border dark:border-slate-500 bg-white dark:bg-slate-700 sm:mx-0 sm:h-10 sm:w-10">
                      ...
                      <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        ...
                        <form id="passwordConfirm" class="mt-4">
                          <input v-model="passwordConfirm" type="password" placeholder="Type your password" class="py-2 px-4 rounded-md border-gray-300 dark:bg-slate-800 dark:border-slate-500/75 dark:text-white w-full focus:outline-hidden focus:ring-sky-500">
                        </form>
                      </div>
                    </div>
                  </div>
                  <div class="px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                    <button type="submit" form="passwordConfirm" :disabled="!checkPassword" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-xs px-4 py-2 bg-sky-600 disabled:bg-opacity-25 text-base font-medium text-white hover:bg-sky-700 focus:outline-hidden focus:ring-2 focus:ring-offset-2 focus:ring-sky-500 sm:ml-3 sm:w-auto sm:text-sm" @click="mainAction">
                      Confirm
                    </button>
                    <button type="button" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-xs px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-hidden focus:ring-2 focus:ring-offset-2 focus:ring-sky-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm" @click="closeModal" ref="cancelButtonRef">
                      Cancel
                    </button>
                  </div>
              </div>
