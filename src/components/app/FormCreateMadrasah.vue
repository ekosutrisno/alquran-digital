<template>
    <form @submit.prevent="$emit('send')" class="mt-4 md:mt-0 w-full max-w-lg space-y-4">
        ...
        <div class="space-x-4 float-right">
            <button type="button" @click="router.push({query: {step: 1}})" class="mt-6 inline-flex justify-center py-2 px-3 border border-transparent shadow-xs text-sm font-medium rounded-md text-white bg-sky-500 hover:bg-sky-400 transition focus:outline-hidden focus:ring-2 focus:ring-offset-2 focus:ring-sky-500">
                <span>Sebelumnya</span>
            </button>
            <button type="submit" class="mt-6 inline-flex justify-center py-2 px-3 border border-transparent shadow-xs text-sm font-medium rounded-md text-white bg-sky-500 hover:bg-sky-400 transition focus:outline-hidden focus:ring-2 focus:ring-offset-2 focus:ring-sky-500">
