<template>
    <form @submit.prevent="()=>{}" class="mt-4 md:mt-0 w-full max-w-lg space-y-4">
        <div>
        ...
        <div>
            ...
            <div class="mt-1 relative rounded-md shadow-sm">
                <input type="email" required autocomplete="off" v-model="data.name" name="email" id="email"
                    class="focus:ring-sky-500 dark:bg-slate-700/25 dark:text-slate-50 focus:border-sky-500 block w-full pl-4 pr-12 sm:text-sm border-slate-300 dark:border-slate-700/50 rounded-md"
        ...
        <div class="space-x-4 float-right">
            <button type="button" class="mt-6 inline-flex justify-center py-2 px-3 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-sky-500 hover:bg-sky-400 transition focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sky-500">
