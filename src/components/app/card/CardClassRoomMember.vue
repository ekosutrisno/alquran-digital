<template>
    <div class="w-full h-full flex flex-col group items-start justify-start p-4 space-y-4 transition bg-white dark:bg-dark-blue border border-slate-700/10 card-shadow-sm rounded dark:border-slate-700/50">
      <!-- Header -->
        <div class="inline-flex space-x-3 items-start w-full">
            ...
            <div class="flex w-full items-start justify-between">
                ...
                <div v-if="member.user_id !== me" class="dark:text-slate-100 text-slate-400 mt-2 inline-flex space-x-4">
                    ...
                    <svg @click="open = !open" xmlns="http://www.w3.org/2000/svg" class="h-7 w-7 hover:text-sky-500 cursor-pointer transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.25">
        ...
        <ProfilePopoverModal :member="member" :open="open" @close-modal="()=>{ open = !open }"/>
    </div>
</template>
...
import ProfilePopoverModal from '../modal/ProfilePopoverModal.vue';
