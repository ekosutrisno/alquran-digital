<template>
    <div :id="`room_member_${member.user_id}`" class="with-transition w-full h-full flex flex-col group items-start justify-start p-4 space-y-4 transition bg-white dark:bg-dark-blue border border-slate-700/10 card-shadow-sm rounded-lg dark:border-slate-700">
      <!-- Header -->
        <div class="inline-flex space-x-3 items-start w-full">
            ...
            <div class="flex w-full items-start justify-between">
                <div class="flex flex-col items-start">
                    ...
                    <p class="text-xs text-slate-500">{{ getPresence(member.user_id).value.state == 'online' ? 'Online' : `Active ${formatDateFromNow(getPresence(member.user_id).value.last_changed)}` }}</p>
                </div>
                <div v-if="member.user_id !== me" class="dark:text-slate-400 text-slate-400 mt-2 inline-flex space-x-2">
                    <svg @click="$router.push({name: 'ClassRoomChatPage', params:{ roomId }})" xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 hover:text-sky-500 cursor-pointer transition-colors hidden md:block" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.25">
